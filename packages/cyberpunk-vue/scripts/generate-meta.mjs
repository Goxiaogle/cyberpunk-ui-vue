...
// Deprecated 别名：在 global.d.ts / web-types.json 中保留这些旧名指向新组件，
// 让 `<旧名>` 模板用法在 IDE 内继续可用，但通过 @deprecated 引导用户迁移。
const DEPRECATED_ALIASES = [
  { from: 'CpDropdown', to: 'CpSelect', since: '1.13.0', reason: 'CpDropdown 已更名为 CpSelect，命名与主流组件库（Element Plus / Ant Design / PrimeVue v4 等）一致。' },
]
...
function generateGlobalTypes(packageName, components) {
  ...
  const componentNameSet = new Set(components.map((item) => item.componentName))
  const aliasLines = DEPRECATED_ALIASES
    .filter((alias) => componentNameSet.has(alias.to))
    .map((alias) => {
      const doc = formatJsDoc(`@deprecated 自 ${alias.since} 起请使用 \`${alias.to}\`。${alias.reason}`)
      const declaration = `    ${alias.from}: typeof import('${packageName}')['${alias.to}']`
      return `${doc}${declaration}`
    })
  const allLines = [...lines, ...aliasLines]
