...
> **实现细节**：关闭按钮内部使用 `CpButton`（`variant="ghost"`），自动继承 Dialog 的 `shape` prop，无需额外配置。
...
| --------- | -------------------------------------------- | ----------------------------------------------- |
| `default` | -                                            | 对话框主体内容                                  |
| `cover`   | -                                            | 封面区域（位于头部上方，与 Card 的 cover 一致） |
| `header`  | -                                            | 自定义整个头部（覆盖 title + 关闭按钮）         |
| `title`   | -                                            | 仅标题区域                                      |
| `footer`  | `{ close: () => void, confirm: () => void }` | 底部操作区域，可通过作用域获取关闭/确认方法     |
...
<!-- 封面插槽 -->
<CpDialog v-model="visible" title="任务详情" type="primary" shape="round">
  <template #cover>
    <img src="/cover.jpg" style="width: 100%; height: 180px; object-fit: cover;" />
  </template>
  <p>封面之下的对话框内容</p>
</CpDialog>

<!-- 作用域 Footer（通过 close / confirm 控制） -->
<CpDialog
  v-model="visible"
  title="操作确认"
  type="success"
...
>
  <p>使用作用域插槽获取 close 和 confirm 方法</p>
  <template #footer="{ close, confirm }">
    <CpButton @click="close">取消</CpButton>
    <CpButton type="success" @click="confirm">确认</CpButton>
  </template>
</CpDialog>
